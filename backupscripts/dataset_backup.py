...
class CEEEnd2EndDataset(Dataset):
    def __init__(
        ...
        for conv_id, utt_map in tqdm(
            self.convs.items(), desc="Building Graph (Explain Supported)"
        ):
            # 排序 utterances
...
#                 # [NEW] Explanation Bridge Node（圖上的 bridge node）
#                 expl_node_idx = num_utts + 3

#                 # [NEW] total_nodes +1（加入 explanation node）
#                 total_nodes = num_utts + 4

#                 # [NEW] explanation node 文字佔位使用 "[EXPL]"（只為了 tokenizer，有需要可之後改成真正 explanation 文本）
#                 current_texts = base_texts + [
#                     "[CLS]",                    # conv node
#                     base_texts[c_idx],         # cause node
#                     base_texts[t_idx],         # target node
#                     "[EXPL]",                  # explanation bridge node
#                 ]
#                 current_spk_ids = base_speaker_ids + [self.spk_map["None"]] * 4
#                 current_emo_ids = base_emotion_ids + [self.emo_map["neutral"]] * 4
...
#                 # -------------------------------------------------
#                 # 5. [NEW] Explanation Bridge Edges (relation type = 6)
#                 #     CauseNode <-> ExplNode <-> TargetNode
#                 # -------------------------------------------------
#                 # 這裡只用 zero_feat，之後若要把 edge-level explain 也灌進來，可以再調整 feat
#                 self._add_edge(
#                     edge_src,
#                     edge_tgt,
#                     edge_types,
...
#                     expl_space_vec=expl_space_vec,
#                     pair_uttpos=torch.tensor([[c_idx, t_idx]], dtype=torch.long),
#                     # [NEW] 把 explanation node 的 index 存進圖裡，之後如果要在 model 裡用得到
#                     expl_node_idx=torch.tensor(expl_node_idx, dtype=torch.long),
#                 )
#                 self.data_list.append(graph_data)
